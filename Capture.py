...
from tkinter import *
cred = credentials.Certificate("face-recognition-f52bc-firebase-adminsdk-yjxmo-61e9a398e8.json")
firebase_admin.initialize_app(cred)
db = firestore.client()

name_dict = dictionary('student_data')

doc_ref = db.collection('student_data').document()
doc_ref.set(name_dict)
...
def record():
    ...
    from dictionary import dictio
    faces_encodings = []

    # Get the details of trained encodings from the file: 'encodings.json'
    with open('Training_Finding_encodings/encodings.json', 'r') as file:
    ...
    def mark_Attendance(name):
        with open('Attendance.csv',
            ...
            if name in name_list:
                ...
                date_time = cur_datetime.strftime('%H:%M:%S')
                t = time.strftime("%d-%m-%Y")
                f.writelines(f'\n{name},{t},{"nil"},{date_time}')
                temp_dict = dictio('Attendance')
            else:
                time.sleep(5)
                messagebox.showerror('Python error', 'Your Attendance is already marked!!\nPress key [esc] to return')
